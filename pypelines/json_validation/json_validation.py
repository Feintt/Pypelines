...
def validate_json_schema(data, schema, _changeset):
    try:
...
def validate_email(data, key, _changeset):
...
def validate_required_fields(data, required_keys, _changeset):
...
def validate_data_type(data, key, expected_type, _changeset):
...
def validate_string_length(data, key, min_length=None, max_length=None, _changeset=None):
    if key in data and isinstance(data[key], str):
...
def validate_pattern(data, key, pattern, _changeset):
    if key in data and isinstance(data[key], str):
...
def validate_numeric_range(data, key, min_value=None, max_value=None, _changeset=None):
...
def validate_any_key_present(data, keys, _changeset):
...
def validate_enum(data, key, valid_values, _changeset):
    if key in data:
...
def validate_nested_json(data, key, validation_pipeline, _changeset):
...
def validate_list_items(data, key, item_type=None, validation_func=None, _changeset=None):
...
def validate_boolean(data, key, _changeset):
    if key in data:
...
def validate_number(data, key, _changeset):
    if key in data:
...
def validate_string(data, key, _changeset):
    if key in data:
...
def validate_dict(data, key, _changeset):
    if key in data:
...
def validate_date(data, key, date_format="%Y-%m-%d", _changeset=None):
...
def validate_list(data, key, _changeset):
    if key in data:
...
def validate_tuple(data, key, _changeset):
    if key in data:
...
def validate_set(data, key, _changeset):
    if key in data:
...
def validate_null(data, key, _changeset):
    if key in data:
...
def validate_function(data, key, _changeset):
    if key in data:
...
def validate_instance(data, key, class_type, _changeset):
    if key in data:
...
def validate_date_range(data, start_date_key, end_date_key, date_format="%Y-%m-%d", _changeset=None):
    from datetime import datetime
    errors = {}
    try:
